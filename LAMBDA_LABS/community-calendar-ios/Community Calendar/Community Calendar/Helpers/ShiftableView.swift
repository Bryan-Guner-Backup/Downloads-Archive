...
class ShiftableView: UIView, UITextFieldDelegate, UITextViewDelegate, UIGestureRecognizerDelegate {
...
//        textView.layer.borderColor = UIColor.gray.cgColor
    ...
    @objc func yShiftWhenKeyboardAppearsFor(textInput: UIView, keyboardSize: CGRect, nextY: CGFloat) -> CGFloat {
        ...
        let textFieldOrigin = self.convert(textInput.frame, from: textInput.superview!).origin.y
        ...
        let maximumY = self.frame.height - (keyboardSize.height + self.safeAreaInsets.bottom)
    ...
    @objc func keyboardWillHide(notification: Notification) {
        ...
        if self.frame.origin.y != 0 {
            ...
            self.frame.origin.y += currentYShiftForKeyboard
        }
